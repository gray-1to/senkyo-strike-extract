...
def operation(
    ...
    striken_ws: openpyxl.worksheet.worksheet.Worksheet,
    no_striken_ws: openpyxl.worksheet.worksheet.Worksheet,
) -> None:
    # check対象の列を特定
    ...
    for row_index in range(3, input_ws.max_row + 1):
        ...
        if striken_detected:
            ...
            for copy_to_col_index in range(1, input_ws.max_column + 1):
                ...
                striken_cell: openpyxl.cell.cell.Cell = striken_ws.cell(
        ...
        # 打ち消し線がcheck対象の列にない場合
            ...
            for copy_to_col_index in range(1, input_ws.max_column + 1):
                original_cell = input_ws.cell(row=row_index, column=copy_to_col_index)
                no_striken_cell: openpyxl.cell.cell.Cell = no_striken_ws.cell(
                    row=copy_to_row_index, column=copy_to_col_index, value=original_cell.value
                )
                # 文字装飾のコピー
